#!/bin/bash
# ==============================================================
# PFC 全量实验 (6 消息大小 × 5 错误率 × 2 流量类型 = 60 组)
# 用法: bash experiments_pfc/run_pfc.sh
# ==============================================================
set -e
cd "$(dirname "$0")/.."

CSV="experiments_pfc/pfc/results.csv"
> "$CSV"
...
for ERRRATE in $ERR_RATES; do
  for MSGSIZE in $MSG_SIZES; do
    for TRAFFIC in $TRAFFICS; do
      ...
      echo ""
      echo "[${done_count}/${total}] PFC | ${TRAFFIC} | ${MSGSIZE} | err=${ERRRATE}"

      TMPLOG=$(mktemp /tmp/sim_XXXXXX.log)
      python3 run.py --topo "$TOPO" --flow "$FLOW" --simul_time "$SIMTIME" --pfc 1 > "$TMPLOG" 2>&1
      RC=$?
      ...
      awk '{print $7}' "$FCT" | sort -n | awk \
        -v pr="pfc" -v tr="$TRAFFIC" -v ms="$MSGSIZE" \
...
echo ""
echo "============================================"
echo "PFC: ${done_count} 完成, ${fail_count} 失败"
echo "结果: ${CSV}"
echo "============================================"
