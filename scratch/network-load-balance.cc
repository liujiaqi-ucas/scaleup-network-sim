...
void PreloadCollectiveFlows() {
    ...
    // --- [新增] ---
    for (auto const& [step, count] : step_total_flows) {
        std::cout << "[Init] Step " << step << " expects " << count << " flows." << std::endl;
    }
    // -------------
    flowf.close();
}
...
int main(int argc, char *argv[]) {
    ...
    // E2E SR 分支：NIC RTO = 4× rto_us（覆盖 E2E RTT = 2×hop RTT）
    uint32_t nic_rto_us = rto_us * 4;
    Config::SetDefault("ns3::QbbNetDevice::RtoValue", TimeValue(MicroSeconds(nic_rto_us)));
    std::cerr << "NIC RTO\t\t\t\t" << nic_rto_us << " us (E2E SR, 4x hop-RTT)\n";
...
std::cout<<"333333333"<<std::endl;
    ...
    for (uint32_t i = 0; i < link_num; i++) {
        ...
        // E2E SR：只在 NIC→Switch 方向施加错误，Switch→NIC 方向无错误
        // switch 收到的错误靠 source NIC 的 RTO 恢复（端侧重传）
        if (error_rate > 0) {
            if (snode->GetNodeType() == 0) {
                // snode=NIC, dnode=Switch: d.Get(0)=NIC receive（Switch→NIC）→ 清空
                DynamicCast<QbbNetDevice>(d.Get(0))->SetReceiveErrorModel(nullptr);
            } else {
                // snode=Switch, dnode=NIC: d.Get(1)=NIC receive（Switch→NIC）→ 清空
