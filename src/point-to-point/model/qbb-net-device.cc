...
int RdmaEgressQueue::GetNextQindex() {  // 从队列对里面选一个队列，返回第一个满足所有条件的 QP
    ...
    m_earliestPacing = Time(0);  // 重置：本轮没有因 pacing 被跳过的 QP
...
void QbbNetDevice::UpdateRtoTimer() {
  ...
  void QbbNetDevice::HandleRtoTimeout() {
      ...
      // SR 超时：只重传 HEAD（窗口中最老的未确认 flit）
      // 接收端已通过 SR 缓存了其他乱序 flit，无需全部重传
...
void QbbNetDevice::ReleaseRxCredit(uint16_t flitsFreed) {
    if (m_qbbEnabled) return; // PFC 模式：不用信用
...
void QbbNetDevice::DequeueAndTransmit(void) {
    ...
    // ============================================================
    // 0. PFC PAUSE/RESUME: 最高优先级（NIC 和 Switch 共用）
    // ============================================================
    if (m_pfcSendFlag) {
        Ptr<Packet> pfcPkt = Create<Packet>(0);
        PauseHeader ph;
        ph.SetTime(m_pfcSendType == 0 ? m_pausetime : 0);
        ph.SetQLen(m_rxBuffer ? m_rxBuffer->GetCount() : 0);
        ph.SetQIndex(m_pfcSendQIndex);
        pfcPkt->AddHeader(ph);
        CommonHeader co;
        co.SetFlitType(FLIT_TYPE_PFC);
        pfcPkt->AddHeader(co);
        m_tracePfc(m_pfcSendType == 0 ? 1 : 0);
        m_pfcSendFlag = false;
        TransmitStart(pfcPkt);
        return;
    }
    Ptr<Packet> p = nullptr;
    // ============================================================
   ...
   if (m_currentLargePacket != nullptr) {
    //std::cout<<"Node "<<m_node->GetId()<<" device "<<m_ifIndex<<"当前有大包正在切片，继续切片"<<std::endl;
        ...
        // 解决 ns-3 Time 纳秒精度截断导致 flit 传输提前完成但 pacing 未过期的问题
    ...
    if (!m_txQueue.empty() && (m_qbbEnabled || remaining > 0)) {// PFC 绕过信用 / CBFC 检查信用
...
void QbbNetDevice::HandleBitmapNAK(uint64_t baseSeq, uint64_t bitmapLow, uint64_t bitmapHigh) {
    // 先做累计确认：baseSeq 之前的包全部确认
      ...
      // 冷却机制：防止每个后续 NAK 都重复排队 baseSeq（isRetransmitting 在 TransmitStart 前清零，
      // 导致后续 NAK 可以反复重入队，实际重传次数≈窗口大小而非1次）。
      // RTO/30 ≈ 1.5×RTT，足以区分"刚重传完在途中"和"超时未到达"两种状态。
      ...
      // ----- 步骤 2：解析 128 位位图 -----
      // bit=1：对端已乱序收到 → 释放存储
      // bit=0：不重传。NAK 在第一个乱序包到达时立即发出，此时后续 flit 尚在链路上传输，
...
void QbbNetDevice::Receive(Ptr<Packet> packet) {
    ...
    if (!m_qbbEnabled && fh.HasCredit()) {  // PFC 模式忽略信用
        fh.setcreditflag(0);
...
uint32_t QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type) {
    // 标志位模式：不直接调用 TransmitStart（可能 BUSY 导致 assert 崩溃）
    // 而是设置标志，让 DequeueAndTransmit 以最高优先级发出
    m_pfcSendFlag = true;
    m_pfcSendType = type;
    m_pfcSendQIndex = (uint8_t)qIndex;
    if (m_txMachineState == READY) {
        DequeueAndTransmit(); // TX 空闲则立即触发
    }
    // TX 忙则等 TransmitComplete → DequeueAndTransmit 自动处理
    return 0;
}
