#!/bin/bash
# ==============================================================
# NVL72 alltoall 16MB BER 实验
# 1消息大小 × 10 BER错误率 = 10 组
#
# 用法: bash experiments_nvl72/run_nvl72_16mb.sh <协议名> <simtime_s> [选项]
#
# 参数:
...
#
# 示例:
#   bash experiments_nvl72/run_nvl72_16mb.sh gbn 2.0
#   bash experiments_nvl72/run_nvl72_16mb.sh pfc 2.0 --pfc 1
#   bash experiments_nvl72/run_nvl72_16mb.sh sr  2.0 --pool 18432 --credit 128
# ==============================================================
cd "$(dirname "$0")/.."

PROTOCOL=${1:?用法: bash experiments_nvl72/run_nvl72_16mb.sh <协议名> <simtime_s> [--pfc 0|1] [--pool N] [--credit N]}
...
mkdir -p "$OUTDIR"
CSV="${OUTDIR}/results_16mb.csv"

echo "协议: ${PROTOCOL}  PFC=${PFC}  pool=${POOL}  credit=${CREDIT}"
echo "消息: alltoall 16MB  simtime=${SIMTIME}s"
echo "结果: ${CSV}"
echo ""
...
echo ""
echo "============================================"
echo "${PROTOCOL} 16MB: ${done_count} 完成, ${fail_count} 失败"
